...
**Current status:** Phase 12 complete. Models updated to latest: Claude Sonnet/Opus 4.6, GPT-5.2/5.3 Codex, Gemini 3.1 Pro (via OpenRouter). Thinking/reasoning enabled for all providers via `providerOptions` (adaptive thinking for Claude, reasoning effort for OpenAI, openrouter.reasoning for Gemini) with `sendReasoning: true` streaming reasoning content to client. `DEFAULT_MODELS` constant centralizes default model selection. PWA installable with `vite-plugin-pwa`, manifest, service worker (stale-while-revalidate for assets, network-first for navigation, API routes excluded). Keyboard shortcuts: Cmd/Ctrl+N (new conversation), Cmd/Ctrl+K (search), Enter to send, Shift+Enter for newline. ConversationSearch dialog with fuzzy title filtering and keyboard navigation. InputBar upgraded to textarea for multiline. Skeleton loaders in sidebar. Responsive touch targets (44px min on mobile). Safe-area padding for iOS. CSS fade-in transition on conversation switch. Request deduplication guard. Accessibility: skip-to-content link, semantic landmarks (section, role=log, role=alert, role=status), ARIA labels on all interactive elements, focus management. Previous phases: Export (Phase 11), token usage (Phase 10), conversation management (Phase 9), cross-feed (Phase 8), settings (Phase 7), tri-model streaming (Phase 6), etc.
